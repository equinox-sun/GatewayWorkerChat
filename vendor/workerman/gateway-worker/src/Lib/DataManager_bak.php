...
class DataManager 
{
       ...
       {
           return self::$db->query('SELECT * FROM `odm_chat_user` where `user`=\''.$user.'\'');
       }
       ...
       {
           $sql="select * from odm_chat_kefu where is_online=1 order by usernum asc limit 1";//获取到对接用户最少当中的一个在线客服
//            $sql="SELECT * FROM `chat_kefu` where isonline=1  ORDER BY RAND() LIMIT 1";//随机获取一个在线客服
       ...
       {
           $sql="select * from `odm_chat_user` where `user`='{$user}'";
           $result=self::$db->query($sql);
           return !empty($result)?$result[0]:array();
       }
       
       /**
        * 根据id_account_number获取用户
        */
       public static function getUserByAccount($account,$id)
       {
          $sql="select * from `odm_users` where `user_id`='{$id}' and account_number = '{$account}'";
          $result=self::$db->query($sql);
           return !empty($result)?$result[0]:array();
       }
       ...
       {
           $sql="select * from `odm_chat_kefu` where `kefu`='{$kefu}'";
           $result=self::$db->query($sql);
           return !empty($result)?$result[0]:array();
       }
       /**
       ...
       public function isHaveKefu($user)
       {
           ...
           if(!empty($userinfo['kefu']))
           {
               ...
               !empty($kefuinfo) && $kefuinfo['is_online'] && $flag=1;
           }
       ...
       {
            $sql="select count(*) as usernum from `odm_chat_user` where `kefu`='{$kefu}'";
            $info=self::$db->query($sql);
            self::save('odm_chat_kefu', !empty($info)?$info[0]:array(),"kefu='{$kefu}'");
       }
       /**
        * 獲取客服所接待的用戶
        * @param unknown $kefu
       ...
       {
           $sql="select * from `odm_chat_user` where `kefu`='{$kefu}' and is_online = 1";
       ...
       {
           $sql="select * from `odm_chat_user` where `is_online`=1 and kefu='' order by last_login asc limit 5";
           return self::$db->query($sql);
       }
       ...
       public function setOnlineUserKefu($kefu)
       {
           ...
           if (!empty(self::getKefuByKefu($kefu))) {
               ...
               foreach ($res as $key=>$val) {
                   self::save('odm_chat_user', $data,"`kefu`='".$val['kefu']."'");
               }
           }
       ...
       public function saveKefuClientId($clients_list,$kefu)
       {
           ...
           $sql="update odm_chat_kefu set clients_id='{$client_ids}',is_online=1 where kefu='{$kefu}'";
           self::$db->query($sql);
       }
       //记录发送的消息
       public function recordMsg($user, $kefu, $msg, $to, $goods_detail = '')
       {
           $data['user'] = $user;
           $data['kefu'] = $kefu;
           $data['msg'] = $msg;
           ...
           self::insert('odm_chat_message', $data);
       }
       ...
       public function setClientsIdAndIsOnline($client_id,$client_name,$iskefu){
           if ($iskefu) {
               ...
               $userInfo = self::getUserByUser($client_name);
               //关闭连接，修改用户在线状态设置成离线
               if (!empty($userInfo)) {
                   $sql = 'update odm_chat_user set is_online=0 where id ='.$userInfo['id'];
                   self::$db->query($sql);
               }
              /* $clients_id = explode(",", $userInfo['clients_id']);
               if (in_array($client_id, $clients_id)) {
                   $clients_id_ = array_flip($clients_id);
                   unset($clients_id[$clients_id_[$client_id]]);
                   $userInfo['clients_id'] = (count($clients_id)>0) ? implode(",", $clients_id) : "";
                   empty($clients_id) && $userInfo['is_online'] = 0;
                   self::save('odm_chat_user', $userInfo,"`user`='{$client_name}'");
                }*/
            }
       }
       ...
       public function updateUserIsOline(){
           $time=time()-25200;
           $sql="update odm_chat_user set clients_id='' ,is_online=0 where last_login<{$time}";
            self::$db->query($sql);
       }
